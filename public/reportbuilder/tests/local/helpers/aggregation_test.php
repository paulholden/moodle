...
use core_reportbuilder\local\aggregation\{base, count, sum};
...
final class aggregation_test extends advanced_testcase {
    ...
    public function test_get_full_classpath(): void {
        $classpath = aggregation::get_full_classpath(count::get_class_name());
        $this->assertEquals('\\' . count::class, $classpath);
    }
    ...
    public function test_get_aggregations(): void {
        ...
        $this->assertContains(count::class, $aggregations);
    ...
    public function test_get_column_aggregations(): void {
        $aggregations = aggregation::get_column_aggregations(column::TYPE_TEXT);
        $this->assertCount(4, $aggregations);

        // Just assert single item from returned structure.
        $this->assertArrayHasKey(count::get_class_name(), $aggregations);
        $this->assertEquals(count::get_name(), $aggregations[count::get_class_name()]);

        // Sum aggregation is never compatible with a text column.
        $this->assertArrayNotHasKey(sum::get_class_name(), $aggregations);

        // Now exclude some aggregation types.
        $aggregations = aggregation::get_column_aggregations(column::TYPE_TEXT, [count::get_class_name()]);
        $this->assertCount(3, $aggregations);
        $this->assertArrayNotHasKey(count::get_class_name(), $aggregations);
    }
}
