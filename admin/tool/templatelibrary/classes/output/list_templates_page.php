...
class list_templates_page implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // Sort returned components according to their type, and name,
        core_collator::asort_objects_by_property($components, 'type');
        array_walk($components, static function(stdClass $component): void {
            core_collator::asort_objects_by_property($component->plugins, 'name');
            $component->plugins = new \ArrayIterator($component->plugins);
        });

        return (object) [
            'allcomponents' => new \ArrayIterator($components),
        ];
    }
}
